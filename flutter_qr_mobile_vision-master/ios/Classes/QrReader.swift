  ...
  init(targetWidth: Int, targetHeight: Int, textureRegistry: FlutterTextureRegistry, options: BarcodeScannerOptions, qrCallback: @escaping(_:[String: Any]) -> Void) throws {
    ...
    if captureDevice == nil {
      ...
      guard captureDevice != nil else {
        ...
        let preset: AVCaptureSession.Preset = AVCaptureSession.Preset.hd1280x720
    ...
    DispatchQueue.global(qos: DispatchQoS.QoSClass.utility).async {
      self.barcodeDetector.process(image) { features, error in
       ...
       for feature in features {
            var ret = [String:Any]();
            ret["displayValue"] =  feature.displayValue;
            ret["left"] =  feature.frame.origin.x;
            ret["top"] =  feature.frame.origin.y;
            ret["width"] =  feature.frame.size.width;
            ret["height"] =  feature.frame.size.height;
            barcodeList.append(ret);
        }
        if(barcodeList.count>0){
            returnVal["containsBarcode"] = true;
        }
        returnVal["barcodes"] = barcodeList;
        self.qrCallback(returnVal)
      }
    }
  }
